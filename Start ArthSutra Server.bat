@echo off
title ArthSutra Server
cd /d "%~dp0"

where node >nul 2>nul
if %errorlevel% neq 0 (
    echo Node.js not found. Run "Install ArthSutra.bat" first.
    pause
    exit /b 1
)
...
echo  ArthSutra running at http://localhost:%ACTIVE_PORT%
...
:opendefault
echo  ArthSutra running at http://localhost:3001
