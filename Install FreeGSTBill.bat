...
title  GST Billing Software - Installer
color 0B
...
if %errorlevel% neq 0 (
    ...
    echo        4. Come back here and run Install GSTBill.bat again.
    echo.
...
set "TARGET_PATH=%~dp0Start GSTBill.bat"

:: Desktop shortcut — points at index.html (the new control panel)
:: rather than Start GSTBill.bat directly. Reason: the .bat fails
...
set "DESKTOP_SHORTCUT=%USERPROFILE%\Desktop\GST Billing Software.lnk"
set "LAUNCHER_PATH=%~dp0index.html"

:: Start Menu shortcut (searchable from Windows Start)
set "STARTMENU_DIR=%APPDATA%\Microsoft\Windows\Start Menu\Programs\ GST Billing Software"
if not exist "%STARTMENU_DIR%" mkdir "%STARTMENU_DIR%"
set "STARTMENU_SHORTCUT=%STARTMENU_DIR%\GST Billing Software.lnk"
set "STARTMENU_LAUNCHER=%STARTMENU_DIR%\ GST Billing.lnk"
...
(
    ...
    echo desktopShortcut.Description = " GST Billing Software — Control Panel (start/stop/open)"
    ...
    echo startShortcut.Description = "GST Billing Software — Control Panel"
    ...
    echo quickShortcut.Description = "GST Billing Software — start server and open app directly"
...
if exist "%STARTMENU_SHORTCUT%" (
    echo         Start Menu shortcut created (search " GST Billing" in Start)
) else (
...
set "STARTUP_SHORTCUT=%STARTUP_DIR%\GST Billing Server.lnk"
...
(
    ...
    echo startupShortcut.Description = "GST Billing Software Server Auto-Start"
...
:: Register gstbill:// protocol (so browser "Start Server" button works)
powershell -Command "New-Item -Path 'HKCU:\Software\Classes\gstbill' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill' -Name '(default)' -Value 'URL: GST Billing Protocol'; New-ItemProperty -Path 'HKCU:\Software\Classes\gstbill' -Name 'URL Protocol' -Value '' -Force | Out-Null; New-Item -Path 'HKCU:\Software\Classes\gstbill\shell\open\command' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill\shell\open\command' -Name '(default)' -Value '%TARGET_PATH%'" 2>nul
echo         Start button registered

:: Register gstbill-update:// protocol (so browser "Update Now" button works)
set "UPDATE_PATH=%~dp0Update GSTBill.bat"
powershell -Command "New-Item -Path 'HKCU:\Software\Classes\gstbill-update' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-update' -Name '(default)' -Value 'URL:GST Billing Update Protocol'; New-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-update' -Name 'URL Protocol' -Value '' -Force | Out-Null; New-Item -Path 'HKCU:\Software\Classes\gstbill-update\shell\open\command' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-update\shell\open\command' -Name '(default)' -Value '%UPDATE_PATH%'" 2>nul
echo         Update button registered

:: Register gstbill-stop:// protocol (so Control Panel "Stop Server" button works)
set "STOP_PATH=%~dp0Stop GSTBill.bat"
powershell -Command "New-Item -Path 'HKCU:\Software\Classes\gstbill-stop' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-stop' -Name '(default)' -Value 'URL:GST Billing Stop Protocol'; New-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-stop' -Name 'URL Protocol' -Value '' -Force | Out-Null; New-Item -Path 'HKCU:\Software\Classes\gstbill-stop\shell\open\command' -Force | Out-Null; Set-ItemProperty -Path 'HKCU:\Software\Classes\gstbill-stop\shell\open\command' -Name '(default)' -Value '%STOP_PATH%'" 2>nul
...
start "" "%~dp0Start GSTBill.bat"

echo.
echo  ========================================================
echo.
echo     GST Billing Software is running!
echo.
echo     HOW IT WORKS:
echo       - Server starts automatically when you turn on PC
echo       - Click "GST Billing Software" on Desktop to open
echo         the Control Panel (start/stop/open the app)
echo       - Or search "GST Billing" in Start Menu
echo       - For quick auto-open (skip the panel) use
echo         "Open GST Billing" in the Start Menu folder
